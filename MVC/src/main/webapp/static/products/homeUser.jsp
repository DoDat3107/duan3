    ...
    <table border="1">
      ...
      <c:forEach var="user" items="${users}">
        <tr>
          ...
          <td>
            <button> <a   href="<c:url value='/menuUser?action=edit&id=${user.id}'/>">Sửa</a></button>
            <button> <a    href="<c:url value='/menuUser?action=delete&id=${user.id}'/>" onclick="return confirm('Bạn có chắc chắn muốn xóa người dùng này không?');">Xóa</a></button>
          </td>
        </tr>
      </c:forEach>
    </table>
