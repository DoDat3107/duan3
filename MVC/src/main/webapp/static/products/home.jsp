...
<div class="container">
    ...
    <div class="menu">
        <div class="box">
<%--            <button><a href="http://localhost:8080/products?action=create">Tạo Sản Phẩm</a></button>--%>
<%--            <button><a href="http://localhost:8080/categories?action=create">Tạo Danh Mục</a></button>--%>

    <div class="select"><a href="<c:url value='/products'/>">Tất cả</a></div>
        ...
        <div class="dropdown">
            <c:forEach var="category" items="${categoryList}">
                <c:if test="${category.name.toLowerCase().contains('áo')}">
                    <a href="<c:url value='/products?action=search&searchCategory=${category.name}'/>">${category.name}</a>
                </c:if>
            </c:forEach>
        </div>
    </div>
    ...
    <div class="select">
        ...
        <div class="login">
            <a href="http://localhost:8080/user?action=login" class="login-btn">Đăng nhập</a>
        </div>
    </div>
    <div class="maketting">
